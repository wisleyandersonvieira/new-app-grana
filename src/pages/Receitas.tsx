...
export default function Receitas() {
  ...
  const totalAReceber = total - totalRecebidas;
  ...
  async function handlePay(ids: string[], dataPagamento: string, contaId: string) {
    ...
    if (paymentBlocked) {
      toast.error(`O mês ${paymentMonth} está bloqueado para recebimentos.`);
    ...
    for (const id of ids) {
      ...
      if (receita && isBlocked(receita.competencia, receita.data_pagamento)) {
        toast.error('Receita bloqueada.');
        continue;
      }
      await supabase.from('receitas').update({ paga: true, data_pagamento: dataPagamento, conta_id: contaId }).eq('id', id);
    }
  ...
  async function handleCancelPayment(id: string) {
    const receita = receitas.find((item) => item.id === id);
    if (receita && isBlocked(receita.competencia, receita.data_pagamento)) {
      toast.error('Não é possível cancelar recebimento em mês bloqueado.');
      return;
    }
  ...
  async function handleDelete(ids: string[]) {
    for (const id of ids) {
      const receita = receitas.find((item) => item.id === id);
      if (!receita) continue;
      if (isBlocked(receita.competencia, receita.data_pagamento)) {
        toast.error('Receita bloqueada.');
        continue;
      }
  ...
  const exportColumns = [
    ...
    { header: 'Recebimento', key: 'pagamento_display' },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Mesma experiência premium da listagem de despesas, adaptada para recebimentos.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => setImportOpen(true)}>
            <FileSpreadsheet className="mr-2 h-4 w-4" /> Importar
          </Button>
          <Button variant="outline" size="sm" onClick={() => exportToExcel(getExportData(), exportColumns, 'receitas')}>
            <FileSpreadsheet className="mr-2 h-4 w-4" /> Excel
          </Button>
          <Button variant="outline" size="sm" onClick={() => exportToPDF(getExportData(), exportColumns, 'Relatório de Receitas', 'receitas')}>
            <FileText className="mr-2 h-4 w-4" /> PDF
          </Button>
          <Button size="sm" onClick={() => navigate('/nova-receita')}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="stat-card">
          <CardContent className="p-5">
            <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Recebidas</p>
            <p className="mt-2 text-2xl font-semibold">{formatCurrency(totalRecebidas)}</p>
          </CardContent>
        </Card>
        <Card className="stat-card">
          <CardContent className="p-5">
            <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Em aberto</p>
            <p className="mt-2 text-2xl font-semibold">{formatCurrency(totalAReceber)}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="listing-filter-grid">
            ...
            <div className="listing-filter-field col-span-12 md:col-span-2">
              ...
              <Select value={draftFilters.situacao} onValueChange={(value) => updateDraft('situacao', value)}>
                ...
                <SelectContent>
                  <SelectItem value="all">Todas</SelectItem>
                  <SelectItem value="pagas">Recebidas</SelectItem>
            ...
            <div className="listing-filter-field col-span-12 md:col-span-4">
              ...
              <div className="relative">
                ...
                <Input className="h-10 pl-9" placeholder="Ex.: cliente, comissão, serviço..." value={draftFilters.descricao} onChange={(e) => updateDraft('descricao', e.target.value)} />
              </div>
            </div>
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border bg-card px-4 py-3 text-sm shadow-sm">
          ...
          <Button size="sm" variant="outline" onClick={() => { setPayIds(Array.from(selected)); setPayModalOpen(true); }}>
            <CheckCircle className="mr-2 h-3.5 w-3.5" /> Receber
          </Button>
          ...
          <table className="listing-table">
            ...
            <thead>
              <tr>
                ...
                {[
                  ...
                  { label: 'Recebimento', field: 'data_pagamento' as SortField, align: 'text-left' },
            ...
            <tbody>
              {!hasSearched ? (
                ...
                <tr>
                  <td colSpan={10} className="px-4 py-10 text-center text-muted-foreground">Nenhuma receita encontrada.</td>
                </tr>
              ) : (
                filtered.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={cn('border-b border-border/60 align-top transition-colors hover:bg-muted/30', item.paga ? 'listing-row-paid' : 'listing-row-open')}>
                      ...
                      <td className="px-3 py-3 text-right whitespace-nowrap font-semibold text-success">{formatCurrency(item.valor)}</td>
                      ...
                      <td className="px-3 py-3 text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => navigate(`/editar-receita/${item.id}`)} disabled={blocked}>
                              <Pencil className="mr-2 h-3.5 w-3.5" /> Editar
                            </DropdownMenuItem>
                            {!item.paga && (
                              <DropdownMenuItem onClick={() => { setPayIds([item.id]); setPayModalOpen(true); }} disabled={blocked}>
                                <CheckCircle className="mr-2 h-3.5 w-3.5" /> Confirmar recebimento
                              </DropdownMenuItem>
                            )}
                            {item.paga && (
                              <DropdownMenuItem onClick={() => handleCancelPayment(item.id)} disabled={blocked}>
                                <XCircle className="mr-2 h-3.5 w-3.5" /> Cancelar recebimento
                              </DropdownMenuItem>
                            )}
      ...
      <AlertDialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Tem certeza que deseja excluir {deleteIds.length} receita(s)?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
